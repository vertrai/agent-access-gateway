...
import (
	...
	"github.com/vertrai/agent-access-gateway/resouces"
)
...
func run(c *cli.Context) (err error) {
	...
	wdb, err := resouces.NewWdb(dsn)
	if err != nil {
		return err
	}
	s := resouces.New(env, resouces.Config{
		...
		TelegramDataDir:                viper.GetString("telegram.dataDir"),
		TelegramMinAvailableBots:       viper.GetInt("telegram.minAvailableBots"),
		TelegramBotName:                viper.GetString("telegram.botName"),
		TelegramBotUsernamePrefix:      viper.GetString("telegram.botUsernamePrefix"),
		TelegramRequestTimeout:         viper.GetDuration("telegram.requestTimeout"),
	}, wdb)
