...
A failure can mean that the Gateway is temporarily unreachable or that automatic Workspace account creation failed. An empty account pool is replenished automatically, so do not suggest manual signup or another account provider. Preserve and report the Gateway error details so the administrator can fix its Google creation credentials or Workspace configuration, but never include the Gateway API key in the report.
